...
namespace MtSparked.UI.Views {
    ...
    public partial class CubePage : ContentPage {
        ...
        public const string GENERATED_DECK_NAME = "Generated Packs";
        public const string NEW_CUBE_DEF = "New Cube Definition";
        public const string OPEN_CUBE_DEF = "Open Cube Definition";
        public const string SAVE_CUBE_DEF_AS = "Save Cube Definition As";
        public const string ACCESS_HELP = "See Reference/Help";

        private IUserDialogs Dialogs { get; } = UserDialogs.Instance;
        ...
        public void OnPackNumChanged(object sender, TextChangedEventArgs args) {
            if(!String.IsNullOrWhiteSpace(args.NewTextValue) && args.NewTextValue != NUMBER_OF_PACKS) {
                if (!Int32.TryParse(args.NewTextValue, out _)) {
                    if (Int32.TryParse(args.OldTextValue, out int _)) {
        ...
        public async void GeneratePacks(object sender, EventArgs args) {
            ...
            Deck generated = new Deck(false) { Name = GENERATED_DECK_NAME };
            ...
            generated.RemoveBoard(Deck.MAINBOARD);
            generated.RemoveBoard(Deck.SIDEBOARD);

            foreach(Deck.BoardInfo info in generated.BoardInfos) {
                info.Editable = info.Visible = info.Name != Deck.MASTER;
            }

            await this.Navigation.PushAsync(new DeckPage(generated, false));
        }
        ...
        public async void ManageCubeDef(object sender, EventArgs args) {
            ...
            if(action == NEW_CUBE_DEF) {
                ...
                if(data is null) {
                    await this.DisplayAlert("Error", "Failed to open Cube Definition", "Okay");
                } else {
