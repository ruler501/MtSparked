...
namespace MtSparked.UI.Views {
	...
	{
        private Card Card;
        private Dictionary<string, Label> FoilLabels { get; } = new Dictionary<string, Label>();
        private Dictionary<string, Label> NormalLabels { get; } = new Dictionary<string, Label>();
        ...
        public void UpdateCounters() {
            ...
            foreach (View child in this.GridView.Children.Where(c => Grid.GetRow(c) >= row).ToList()) {
                _ = this.GridView.Children.Remove(child);
            }
        ...
        private void RemoveCard(string board, bool normal) => ConfigurationManager.ActiveDeck.RemoveCard(this.Card, board, normal);

        private void AddCard(string board, bool normal) => ConfigurationManager.ActiveDeck.AddCard(this.Card, board, normal);
    }
}